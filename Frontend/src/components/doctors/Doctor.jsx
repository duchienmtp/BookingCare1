...
import "./Doctor.scss";
import { selectDoctorBookingPackages } from "../../redux/slices/appSlice";
...
function Doctor(props) {
  ...
  const doctorBookingPackages = _.isEmpty(useSelector(selectDoctorBookingPackages)) ? packageInfo.bookingPackages : useSelector(selectDoctorBookingPackages);

  // Maintain original modal functions
  const openModal = () => setIsModalOpen(true);
  const closeModal = () => setIsModalOpen(false);

  // Keep original effect for initial date selection
  useEffect(() => {
    if (scheduleDates?.length > 0 && schedules?.length > 0) {
      // 1. Check valid current date
      let validDate = scheduleDates.find(
        (d) =>
          d.dateStr === currentDate?.dateStr &&
          schedules.some((s) => s.scheduleDate === d.dateStr)
      );

      // 2. Fallback to first available date
      if (!validDate) {
        validDate = scheduleDates.find((d) =>
          schedules.some((s) => s.scheduleDate === d.dateStr)
        );
      }

      // 3. Handle today's schedule priority
      if (!validDate) {
        const todaySchedule = scheduleDates.find((d) =>
          d.dayShowing?.includes("Hôm nay")
        );

        if (todaySchedule) validDate = todaySchedule;
      }

      // Update date if needed
      if (validDate && validDate.dateStr !== currentDate?.dateStr) {
        onDateChange(validDate, packageInfo.id);
      }
    }
  }, [scheduleDates, schedules, currentDate, onDateChange]);

  // Maintain original click-outside handler
  useEffect(() => {
  ...
  return (
    <>
      <div className={`individual-doctor-item ${className}`}>
        {/* Maintain 100% original doctor info section */}
        <div className="doctor-info-section">
          <div className="doctor-info-summary">
            <div className="doctor-image">
              <div className="img-container">
                <img src={packageInfo?.image} alt={packageInfo?.fullName} />
              </div>
            </div>
            <div className="doctor-info-summary-content">
              <div className="page-header">
                {className !== "list-type" ? (
                  ...
                  <Link to={`/dich-vu-y-te/${packageInfo?.slug}`}>
                    <h1 className="page-title">
                      <div className="favorite-section">
                        <div className="img-container">
                          <svg
              ...
              <div className="achievement-section">
                <span>
                  {Array.isArray(packageInfo?.shortPackageInfo)
                    ? packageInfo?.shortPackageInfo.map((item, index) => (
                      <React.Fragment key={index}>
                        {item}
                        <br />
                      </React.Fragment>
                    ))
                    : packageInfo?.shortPackageInfo}
                </span>
              </div>
              <div className="place-section">
                ...
                <span>{packageInfo?.place}</span>
              </div>
            </div>
          </div>
        </div>

        {/* Original booking section structure */}
        {packageInfo?.isDeleted || !hasSchedules ? (
          ...
          <div className="booking-section">
            <div className="left-content">
              <div className="schedule-this-week-dropdown" onClick={openModal}>
                <div className="schedule-title">
                  <span>{currentDate?.displayText}</span>
                </div>
                <div className="img-container">
                  <svg
                    viewBox="0 0 24 24"
              ...
              <div className="schedule-list">
                {schedules?.map((item) => (
                  <div
                    className={`schedule-item ${!item.isValid ? "cannot-book" : "can-book"
                      }`}
                    key={item.scheduleId}
                  >
                    <Link to={`/dat-lich-kham/${item.packageScheduleId}`}>
              ...
              <div className="booking-instruction">
                <span>Chọn&nbsp;</span>
                <div className="img-container">
                  <svg viewBox="0 0 448 512" width={14} height={16} fill="#333">
            ...
            {/* Original right content structure */}
            <div className="right-content">
              <div className="address-section">
                ...
                <div className="clinic-name">
                  <span>{packageInfo?.clinic[0].clinicFullName}</span>
                </div>
                {packageInfo?.clinic[0].clinicAddress
                  ?.split('"/"')
                  .map((item, index) => (
              ...
              <div className="price-section">
                <div className="price-section-container">
                  <div className="price">
                    <span>giá khám:&nbsp;</span>
                    <span>
                      {doctorBookingPackages?.bookingPackagesPriceSummary}
                    </span>
                  </div>
                  <div
                    className="see-detail"
                    style={{ display: isPriceDetailOpen ? "none" : "block" }}
                  >
                    <span onClick={() => setIsPriceDetailOpen(true)}>
                      &nbsp;Xem chi tiết
                    </span>
                  </div>
                </div>
                {isPriceDetailOpen && (
                  <div className="price-section-detail">
                    <div className="price-detail-table">
                      {doctorBookingPackages?.bookingPackages?.map((pkg) => (
                        <div
                          className="price-detail-row"
                          key={pkg.bookingPackageId}
                        >
                          <div className="price-detail-booking-package-name">
                            <span>{pkg.bookingPackageName}</span>
                          </div>
                          <div className="price-detail-booking-package-price">
                            <span>{pkg.bookingPackagePrice}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                    <div className="hide-detail">
                      <span onClick={() => setIsPriceDetailOpen(false)}>
                        Ẩn bảng giá
                      </span>
                    </div>
                  </div>
          ...
          <div className={`schedule-modal ${isModalOpen ? "open" : ""}`}>
            <div className="schedule-list">
              {scheduleDates?.map((dateItem) => (
                <div className="button-container" key={dateItem.date}>
                  <button
                    onClick={() => {
                      onDateChange({
                        date: dateItem.date,
                        dateStr: dateItem.dateStr,
                        displayText: dateItem.displayText, // Maintain original format
                      }, packageInfo.id);
                      closeModal();
                    }}
                  >
                    {dateItem.dayShowing || dateItem.displayText}
                  </button>
                </div>
              ))}
